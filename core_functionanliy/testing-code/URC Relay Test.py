...
led_pins = {"red": 26, "green": 19, "blue": 13,}  # Add white LED pin for flashing
...
def toggle_docker_state():
    os.chdir(docker_compose_dir)  # Change working directory to where docker-compose.yml is located
    if os.path.exists(docker_state_file):
        ...
        with open(docker_state_file, "w") as file:
            file.write("enabled")
        subprocess.call(["docker-compose", "up", "--build", "-d"])  # Start Docker Compose project
